...
// base factory
//----------------------------
UIFactory::UIFactory(QObject *parent) : QObject(parent) {
...
// Window
//----------------------------
Window::Window() : uiObject(new QMainWindow) {
  view = new View();
...
// View
//----------------------------
View::View() : uiObject(new QWidget) { uiObject->setLayout(new QVBoxLayout()); }
...
void View::relayout() {
  ...
  if (align == "flex-start" || align == "center" || justify == "center" ||
      justify == "space-around") {
    l->insertStretch(-1, 1);
  }
  if (align == "flex-end" || align == "center" || justify == "center" ||
      justify == "space-around") {
    l->insertStretch(0, 1);
  }
...
// ScrollView
//----------------------------
ScrollView::ScrollView() : uiObject(new QScrollArea) {
  view = new QWidget();
...
// TextInput
//----------------------------
TextInput::TextInput() : uiObject(new QLineEdit) {
  uiObject->setLayout(new QVBoxLayout());
...
void TextInput::onChange(QString value) {
  ...
  QString script = "$events[\"" + id + "\"].onChange({ target: { src: \"" + id +
                   "\", value: \"" + value + "\" }})";
...
void TextInput::onSubmit() {
  QString id = property("id").toString();
  QString value = ""; //
  QString script = "$events[\"" + id + "\"].onSubmit({ target: { src: \"" + id +
                   "\", value: \"" + value + "\" }})";
  engine->runScript(script);
}
...
// Button
//----------------------------
Button::Button() : uiObject(new QPushButton) {
...
void Button::onClick(bool checked) {
  QString id = property("id").toString();
  QString value = ""; //
  QString script = "$events[\"" + id + "\"].onClick({ target: { src: \"" + id +
                   "\", value: \"" + value + "\" }})";
  qDebug() << script;
  engine->runScript(script);
}
